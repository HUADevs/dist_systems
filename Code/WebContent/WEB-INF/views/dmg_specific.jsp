						...
						<c:forEach items="${dmg_forms}" var="dmg">
							<c:choose>
								<c:when test="${dmg.damageCost > 2000 and empty dmg.approval}">
									<sec:authorize
										access="hasAnyAuthority('Damage Approval more than 2000')">
										<tr>
											<td>${dmg.id}</td>
							...
							value="/cms/damage/${dmg.id}/review"/>"
								...
								<c:when test="${dmg.damageCost > 300 and empty dmg.approval}">
									<sec:authorize
										access="hasAnyAuthority('Damage Approval 300-2000') && !hasAnyAuthority('Damage Approval more than 2000')">
										<tr>
											<td>${dmg.id}</td>
											<td>${dmg.licensePlate.licensePlate}</td>
											<td>${dmg.damageCost}<i class="glyphicon glyphicon-euro"></i></td>
							...
							value="/cms/damage/${dmg.id}/review"/>"
								...
								<c:when test="${dmg.damageCost <= 300 and empty dmg.approval}">
									<sec:authorize
										access="hasAnyAuthority('Damage Approval less than 300') && !hasAnyAuthority('Damage Approval 300-2000') && !hasAnyAuthority('Damage Approval more than 2000')">
										<tr>
											<td>${dmg.id}</td>
											<td>${dmg.licensePlate.licensePlate}</td>
											<td>${dmg.damageCost}<i class="glyphicon glyphicon-euro"></i></td>
